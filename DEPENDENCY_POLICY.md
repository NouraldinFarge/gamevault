...
Production JavaScript dependencies are checked with `pnpm audit --prod`; the complete Rust lockfile is checked with `cargo audit`. Informational maintenance warnings are reviewed separately from exploitable advisories. GameVault is Windows-only, so warnings confined to Tauri's non-shipped GTK/Linux graph are documented and revisited during Tauri upgrades rather than presented as Windows release vulnerabilities.
